...
export const TheatreSeeder = createContext<TheatreSeederValue>({
  theatres: [],
  loading: true
});
...
export const TheatreSeederProvider = ({
  children,
  apiKey
}: {
  ...
  useEffect(() => {
    async function fetchTheatreData() {
      ...
      try {
        ...
        // --- NEW: log static JSON for seeding ---
        const seederData = detailedTheatres.map(theatre => ({
          id: theatre.id,
          name: theatre.name,
          vicinity: theatre.vicinity,
          ...
          screens: theatre.screens.map(screen => ({
            ...
            movie: {
              ...
              backdropPath: screen.movie.backdropPath,
              poster_path: screen.movie.poster_path,
              genres: screen.movie.genres,
              synopsis: screen.movie.synopsis,
              runtime: screen.movie.runtime,
              release_date: screen.movie.release_date,
              tagline: screen.movie.tagline,
              status: screen.movie.status,
              vote_average: screen.movie.vote_average
            },
            type: {
              projector: screen.type.projector,
              tagline: screen.type.tagline,
        ...
        console.log("Seeder JSON:");
        console.log(JSON.stringify(seederData, null, 2));
        // You can copy this output to a JSON file for your seeder
      } catch (err) {
